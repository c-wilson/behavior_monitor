...
from .models import Mouse, Session, Trial
...
def index(request, msg=None):
    mice_list = Mouse.objects.filter(active=True).order_by('-date_added')
...
def inactives(request):
    mice_list = Mouse.objects.all().order_by('-date_added')

    return render(request, 'behavior_subjects/inactives.html', {"mice_list": mice_list})
...
def mouse(request, mouse_number):
    ...
    # y = []
    # x = []
    x = list(sessions.values_list('performance', flat=True))
    y = sessions.values_list('run_dtg', flat=True)
    # for s in sessions:
...
def calc_inh_delay(request, mouse_number, session_num):
    s = Session.objects.get(id=session_num)
    s.calc_inh_delay()
    return redirect('behavior_subjects:mouse', mouse_number)
